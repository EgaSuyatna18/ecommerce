      ...
      <div class="card my-4">
        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
          <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
            <h6 class="text-white text-capitalize ps-3">Buyer table</h6>
          </div>
              ...
              <tbody>
                @foreach ($buyers as $buyer)
                    <tr>
                        ...
                        <td>
                        <span class="text-secondary text-xs font-weight-bold">{{ $buyer->created_at }}</span>
                        </td>
                        <td class="align-middle">
                            <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#editModal"
                              onclick="setData('{{ $buyer->id }}', '{{ $buyer->name }}', '{{ $buyer->email }}')">
                              <i class="material-icons">edit</i>
                            </button>
                            <form action="/buyer/{{ $buyer->id }}" method="post" class="d-inline">
                              @csrf
                              @method('delete')
                              <button type="submit" class="btn btn-danger" onclick="return confirm('Delete Data {{ $buyer->name }} ?')"><i class="material-icons">delete</i></button>
                            </form>
                        </td>
                    </tr>
                @endforeach
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="/buyer" method="post" id="addForm">
          @csrf
          <div class="mb-3">
            <label>Name</label>
...
<script>
  function setData(buyer_id, name, email) {
    editForm.action = '/buyer/' + buyer_id;
    editName.value = name;
